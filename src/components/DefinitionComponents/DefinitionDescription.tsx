...
const DefinitionDescription = () => {
  ...
  const mainDefinition = definitions.length > 0 ? definitions[0] : null;
  ...
  const dictionaryItem = useSelector((state: RootState) =>
    state.dictionary.items.find((item: DictionaryItem) => upperCase(item.word) === upperCase(word) || upperCase(item.translated) === upperCase(word))
  );
  ...
  const onClickDictionary = () => {
    if (!dictionaryItem) {
      dispatch(
        getDictionaryItem({word: word, transcription: transcription})
      );
    }
  };
    ...
    <div className={word && word !== "." ? "description" : "description hidden"}>
      <div className="description__headline">
        <h2 className="description__word">
          {word && upperCase(word)}
          {transcription && (
            <span className="description__transcription">[{transcription}]</span>
          )}
        </h2>
        {sound ? (
          <StartIcon
